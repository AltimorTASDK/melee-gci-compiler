...
import mgc.mgc_file as mgc_file
...
def _cleanup(script_path):
    """Delete temp files generated by the ASM compiler"""
    if not script_path: return
    (mgc_file.tmp_directory/"code.txt").unlink(missing_ok=True)
    (mgc_file.tmp_directory/"code.bin").unlink(missing_ok=True)
    (mgc_file.tmp_directory/"src1.o").unlink(missing_ok=True)
    (mgc_file.tmp_directory/"src2.o").unlink(missing_ok=True)
    # Remove tmp directory only if empty
    try: mgc_file.tmp_directory.rmdir()
    except: pass
    return
